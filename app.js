...
let firebase=null, db=null, auth=null, user=null, demo=false, children=[], exercises=[], sessions=[], modalType='',activeExercise=null,questionIndex=0,exerciseScore=0,answerLocked=false;
const questionBank=[
 {instruction:'Choisis la lettre demandée',prompt:'Trouve la lettre ب',choices:['ت','ب','ث'],answer:'ب'},
 {instruction:'Reconnaissance visuelle',prompt:'Trouve la lettre ث',choices:['ب','ن','ث'],answer:'ث'},
 {instruction:'Complète le mot',prompt:'ـاب',choices:['ب','ت','ك'],answer:'ب'},
 {instruction:'Choisis la bonne syllabe',prompt:'مَـ',choices:['مُ','مَ','مِ'],answer:'مَ'},
 {instruction:'Lettres ressemblantes',prompt:'Quelle lettre a trois points ?',choices:['ت','ب','ث'],answer:'ث'}
];
...
function exerciseHTML(e){return `<article class="exercise-card" data-exercise="${safe(e.id)}" tabindex="0" role="button"><div class="exercise-art" dir="rtl">${safe(e.symbol||'أ')}</div><div><span class="tag">${safe(e.level||'Facile')}</span><h3>${safe(e.title)}</h3><p>${safe(e.skill||'Apprentissage arabe')}</p><small>5 questions · résultat enregistré</small></div></article>`}
function renderExercises(){const available=exercises.length?exercises:demoExercises;$('#recommendedGrid').innerHTML=available.slice(0,4).map(exerciseHTML).join('');$('#exerciseLibrary').innerHTML=available.map(exerciseHTML).join('');$$('[data-exercise]').forEach(card=>{const start=()=>startExercise(available.find(e=>e.id===card.dataset.exercise));card.onclick=start;card.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();start()}}})}

function startExercise(exercise){activeExercise=exercise;questionIndex=0;exerciseScore=0;answerLocked=false;$('#exerciseRunner').classList.remove('hidden');renderQuestion()}
function renderQuestion(){const q=questionBank[questionIndex];$('#runnerStep').textContent=`Question ${questionIndex+1}/${questionBank.length}`;$('#runnerScore').textContent=`${exerciseScore} point${exerciseScore>1?'s':''}`;$('#runnerProgress').style.width=`${((questionIndex+1)/questionBank.length)*100}%`;$('#runnerBody').innerHTML=`<div class="question"><small>${safe(activeExercise.title)} · ${safe(q.instruction)}</small><h2>${safe(q.prompt)}</h2><div class="arabic-prompt" dir="rtl">${safe(activeExercise.symbol||q.answer)}</div><div class="answers" dir="rtl">${q.choices.map(c=>`<button class="answer" data-answer="${safe(c)}">${safe(c)}</button>`).join('')}</div><div id="answerFeedback" class="feedback"></div></div>`;answerLocked=false;$$('[data-answer]').forEach(b=>b.onclick=()=>checkAnswer(b,q))}
function checkAnswer(button,q){if(answerLocked)return;answerLocked=true;const correct=button.dataset.answer===q.answer;if(correct)exerciseScore++;button.classList.add(correct?'correct':'wrong');if(!correct){const right=$$('[data-answer]').find(b=>b.dataset.answer===q.answer);right?.classList.add('correct')}const feedback=$('#answerFeedback');feedback.textContent=correct?'Bravo ! Bonne réponse':'Regarde la réponse en vert';feedback.className=`feedback ${correct?'good':'bad'}`;$('#runnerScore').textContent=`${exerciseScore} point${exerciseScore>1?'s':''}`;setTimeout(()=>{questionIndex++;if(questionIndex<questionBank.length)renderQuestion();else finishExercise()},900)}
async function finishExercise(){const percent=Math.round(exerciseScore/questionBank.length*100);$('#runnerProgress').style.width='100%';$('#runnerStep').textContent='Terminé';$('#runnerBody').innerHTML=`<div class="result"><div class="result-icon">★</div><h2>${percent>=80?'Excellent travail !':percent>=50?'Bravo, continue !':'On réessaie ensemble'}</h2><p>${safe(activeExercise.title)} terminé</p><div class="result-score">${percent}%</div><p>${exerciseScore} bonne${exerciseScore>1?'s':''} réponse${exerciseScore>1?'s':''} sur ${questionBank.length}</p><div class="result-actions"><button id="retryExercise" class="secondary">Recommencer</button><button id="finishExercise" class="primary">Terminer</button></div></div>`;try{await save('results',{exerciseId:activeExercise.id,exerciseTitle:activeExercise.title,score:percent,correctAnswers:exerciseScore,totalQuestions:questionBank.length,completedAt:new Date().toISOString()})}catch(e){console.error(e);toast('Résultat affiché mais non enregistré')}$('#retryExercise').onclick=()=>startExercise(activeExercise);$('#finishExercise').onclick=closeRunner}
function closeRunner(){$('#exerciseRunner').classList.add('hidden');activeExercise=null}
